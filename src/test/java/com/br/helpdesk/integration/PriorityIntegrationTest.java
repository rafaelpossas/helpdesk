...
import com.br.helpdesk.controller.PriorityController;
...
public class PriorityIntegrationTest {
    ...
    public void testFindAll() throws Exception {
        MvcResult mvcResult = mockMvc.perform(MockMvcRequestBuilders.get("/priority"))
        ...
        assertThat(jsonArray.length(), is(5));// verifica se a quantidade de objetos retornados � igual a 2, isso varia de acordo com a configura��o do "PriorityData.xml"
    }
    ...
    public void testFindByIdEntityNotFound() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.get("/priority/{id}", 50L))
                .andExpect(MockMvcResultMatchers.status().isNotFound());//verifica se o retorno � entityNotFound    
    }
    
    @Test
    public void testFindByNameContainingOk() throws Exception {
        MvcResult mvcResult = mockMvc.perform(MockMvcRequestBuilders.get("/priority").param("name", "EXISTE_ENTIDADE_COM_ESSE_NOME"))
    ...
    public void testFindByIdOk() throws Exception {
        MvcResult mvcResult = mockMvc.perform(MockMvcRequestBuilders.get("/priority/{id}", 5L))
                .andExpect(MockMvcResultMatchers.status().isOk())//verifica se o retorno � ok 
                .andExpect(MockMvcResultMatchers.content().contentType(TestUtil.APPLICATION_JSON_UTF8))//verifica se esta retornando um JSON na codifica��o UTF8;  
                .andReturn();
    ...
    public void testDeleteEntityNotFound() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.delete("/priority/{id}", 50L))
                .andExpect(MockMvcResultMatchers.status().isNotFound());//verifica se o retorno � entityNotFound    
    }
    ...
    public void testDeleteEntity() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.delete("/priority/{id}", 5L))
    ...
    public void testSaveNewEntity() throws Exception{
        ...
        priority.setName("New Priority");
        
        MvcResult mvcResult = mockMvc.perform(MockMvcRequestBuilders.put("/priority")
        ...
        assertThat(jsonObject.getString("name"), is("New Priority"));        
    }
