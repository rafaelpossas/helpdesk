...
import com.br.helpdesk.controller.UserGroupController;
...
public class UserGroupIntegrationTest {
    ...
    public void testFindAll() throws Exception {
        MvcResult mvcResult = mockMvc.perform(MockMvcRequestBuilders.get("/group"))
        ...
        assertThat(jsonArray.length(), is(5));// verifica se a quantidade de objetos retornados � igual a 2, isso varia de acordo com a configura��o do "UserGroupData.xml"
    }
    
    
    @Test
    public void testFindByNameContainingNotFound() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.get("/group").param("name", "NAO_EXISTE_ENTIDADE_COM_ESSE_NOME"))
    ...
    public void testFindByIdEntityNotFound() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.get("/group/{id}", 50L))
                .andExpect(MockMvcResultMatchers.status().isNotFound());//verifica se o retorno � entityNotFound    
    }
    
    @Test
    public void testFindByNameContainingOk() throws Exception {
        MvcResult mvcResult = mockMvc.perform(MockMvcRequestBuilders.get("/group").param("name", "EXISTE_ENTIDADE_COM_ESSE_NOME"))
    ...
    public void testFindByIdOk() throws Exception {
        MvcResult mvcResult = mockMvc.perform(MockMvcRequestBuilders.get("/group/{id}", 5L))
                .andExpect(MockMvcResultMatchers.status().isOk())//verifica se o retorno � ok 
                .andExpect(MockMvcResultMatchers.content().contentType(TestUtil.APPLICATION_JSON_UTF8))//verifica se esta retornando um JSON na codifica��o UTF8;  
                .andReturn();
    ...
    public void testDeleteEntityNotFound() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.delete("/group/{id}", 50L))
                .andExpect(MockMvcResultMatchers.status().isNotFound());//verifica se o retorno � entityNotFound    
    }
    ...
    public void testDeleteEntity() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.delete("/group/{id}", 5L))
    ...
    public void testSaveNewEntity() throws Exception{
        ...
        group.setName("New UserGroup");
        
        MvcResult mvcResult = mockMvc.perform(MockMvcRequestBuilders.put("/group")
        ...
        assertThat(jsonObject.getString("name"), is("New UserGroup"));        
    }
